...
use super::{
    ...
    predicate_occurence_semantics::{PredicateOccurenceLocation, PredicateOccurenceSemantics},
};
use dashmap::DashMap;
...
impl PredicateSemantics {
     ...
     */
    pub fn insert_predicate_for_node(
        semantics: &EncodingSemantics,
        identifier: String,
        arity: usize,
        new_value: PredicateOccurenceSemantics,
    ) {
        if semantics
            .predicate_semantics
            .predicates
            .contains_key(&(identifier.clone(), arity))
        {
            semantics
                .predicate_semantics
                .predicates
                .get_mut(&(identifier, arity))
                .unwrap()
                .insert(new_value);
            return;
        }
        ...
        match node.kind() {
            "atom" | "term" => {
                    ...
                    && node.child(0).unwrap().kind() == "identifier"
                {
                    ...
                    Self::insert_predicate_for_node(
                        &document.semantics,
                        identifier,
                        arity,
                        PredicateOccurenceSemantics {
                            node_id: node.id(),
                            range: node.range(),
                            location,
                        },
            ...
            "termvec" | "argvec" => {
                ...
                for child in node.children(&mut node.walk()) {
                    arity += document
                        .semantics
                        .predicate_semantics
                        .get_predicates_arity_for_node(&child.id());
                }
                document
                    .semantics
                    .predicate_semantics
                    .predicates_arity
                    .insert(node.id(), arity);
            }
            "COMMA" => {
                document
                    .semantics
