...
use super::{error_semantic::ErrorSemantic, encoding_semantic::{Semantics, EncodingSemantics}, missing_semantic::MissingSemantic, term_semantic::{TermSemantic, TermType}, special_literal_semantic::SpecialLiteralSemantics};
...
impl StatementSemantics {
     ...
     */
    pub fn with_dependencies(mut self, dependencies: Vec<(HashSet<String>, HashSet<String>)>) -> StatementSemantics {
     ...
     */
    pub fn with_special_literals(mut self, special_literals: Vec<SpecialLiteralSemantics>) -> StatementSemantics {
     ...
     */
    pub fn update_global_vars_for_node(semantics: &EncodingSemantics, node_id: usize, new_value: HashSet<String>) {
        if semantics.statement_semantics.contains_key(&node_id) {
            semantics.statement_semantics.get_mut(&node_id).unwrap().global_vars = new_value;
            return;
        }

        semantics.statement_semantics.insert(node_id, StatementSemantics::new().with_global_vars(new_value));
    }
    ...
    pub fn update_provide_for_node(semantics: &EncodingSemantics, node_id: usize, new_value: HashSet<String>) {
        ...
        semantics.statement_semantics.insert(node_id, StatementSemantics::new().with_provide(new_value));
    }

    /**
     * Update depend for a node, if there is no statement semantics object for that node it creates one
     */
    pub fn update_depend_for_node(semantics: &EncodingSemantics, node_id: usize, new_value: HashSet<String>) {
        if semantics.statement_semantics.contains_key(&node_id) {
            semantics.statement_semantics.get_mut(&node_id).unwrap().depend = new_value;
            return;
        }

        semantics.statement_semantics.insert(node_id, StatementSemantics::new().with_depend(new_value));
    }

    /**
     * Update dependencies for a node, if there is no statement semantics object for that node it creates one
     */
    pub fn update_dependencies_for_node(semantics: &EncodingSemantics, node_id: usize, new_value: Vec<(HashSet<String>, HashSet<String>)>) {
        if semantics.statement_semantics.contains_key(&node_id) {
            semantics.statement_semantics.get_mut(&node_id).unwrap().dependencies = new_value;
            return;
        }

        semantics.statement_semantics.insert(node_id, StatementSemantics::new().with_dependencies(new_value));
    }

    /**
     * Update special literals for a node, if there is no statement semantics object for that node it creates one
     */
    pub fn update_special_literals_for_node(semantics: &EncodingSemantics, node_id: usize, new_value: Vec<SpecialLiteralSemantics>) {
        if semantics.statement_semantics.contains_key(&node_id) {
            semantics.statement_semantics.get_mut(&node_id).unwrap().special_literals = new_value;
            return;
        }
    ...
    fn pass_on_global_vars_from_children(node: Node, document: &mut DocumentData) {
        ...
        for child in node.children(&mut node.walk()) {
            global_vars_in_children.extend(document.semantics.get_statement_semantics_for_node(child.id()).global_vars);
        }
    ...
    fn pass_on_provide_from_children(node: Node, document: &mut DocumentData) {
        ...
        for child in node.children(&mut node.walk()) {
            provide_in_children.extend(document.semantics.get_statement_semantics_for_node(child.id()).provide);
        }
    ...
    fn pass_on_depend_from_children(node: Node, document: &mut DocumentData, remove_provide: bool) {
        ...
        for child in node.children(&mut node.walk()) {
            depend_in_children.extend(document.semantics.get_statement_semantics_for_node(child.id()).depend);
        }

        if remove_provide {
            depend_in_children = depend_in_children.difference(&document.semantics.get_statement_semantics_for_node(node.id()).provide).cloned().collect();
        }
     ...
     */
    fn pass_on_dependencies_from_children(node: Node, document: &mut DocumentData, check_for_node_kind: &HashSet<String>) {
        ...
        for child in node.children(&mut node.walk()) {
            ...
            if check_for_node_kind.len() == 0 || check_for_node_kind.contains(child.kind()) {
                dependencies_in_children.extend(document.semantics.get_statement_semantics_for_node(child.id()).dependencies);
            }
            
        }

        Self::update_dependencies_for_node(&document.semantics, node.id(), dependencies_in_children);
    }
    ...
    fn pass_on_special_literals_from_children(node: Node, document: &mut DocumentData) {
        ...
        for child in node.children(&mut node.walk()) {
            special_literals_in_children.extend(document.semantics.get_statement_semantics_for_node(child.id()).special_literals);
        }

        Self::update_special_literals_for_node(&document.semantics, node.id(), special_literals_in_children);
    }
     ...
     */
    fn pass_on_provide_from_specific_node(document: &mut DocumentData, to_be_updated: Node, from_node: Node) {
        Self::update_provide_for_node(&document.semantics, to_be_updated.id(), document.semantics.get_statement_semantics_for_node(from_node.id()).provide);
    }
                    ...
                    match operator.kind() {
                        ...
                        "ADD" | "SUB" => {
                            ...
                            if Self::is_evaluable(left_child.id(), document) {
                                ...
                                Self::pass_on_provide_from_specific_node(document, node, right_child);
                                ...
                                Self::pass_on_provide_from_specific_node(document, node, left_child);
                            }
                        }
                        "MUL" => {
                            // We have an term of form a * b
                            document.semantics.get_statement_semantics_for_node(left_child.id()).term.value.contains(&0);
                            if Self::is_evaluable(left_child.id(), document) && !document.semantics.get_statement_semantics_for_node(left_child.id()).term.value.contains(&0) {
                                // If the left term is a constant then we pass on the provide value of the right child
                                Self::pass_on_provide_from_specific_node(document, node, right_child);
                            } else if Self::is_evaluable(right_child.id(), document) && !document.semantics.get_statement_semantics_for_node(right_child.id()).term.value.contains(&0) {
                                // If the right term is a constant then we pass on the provide value of the left child
                                Self::pass_on_provide_from_specific_node(document, node, left_child);
                            }
                        }
            ...
            "argvec" => {
                ...
                if node.child_count() == 1 {
                    ...
                    if semicolon.kind() == "SEM" {
                        ...
                        let left_provide = document.semantics.get_statement_semantics_for_node(left_child.id()).provide;
                        let right_provide = document.semantics.get_statement_semantics_for_node(right_child.id()).provide;
            ...
            "argvec" => {
                ...
                if node.child_count() == 1 {
                    ...
                    if semicolon.kind() == "SEM" {
                        ...
                        let left_depend = document.semantics.get_statement_semantics_for_node(left_child.id()).depend;
                        let right_depend = document.semantics.get_statement_semantics_for_node(right_child.id()).depend;
    
                        let mut depend : HashSet<String> = left_depend.union(&right_depend).cloned().collect();
    
                        //Remove any variables who are provided
                        depend = depend.difference(&document.semantics.get_statement_semantics_for_node(node.id()).provide).cloned().collect();
                ...
                if node.child_count() == 1 {
                    ...
                    match operator.kind() {
                        ...
                        _ => {
                            ...
                            let mut depend = document.semantics.get_statement_semantics_for_node(node.id()).vars;
                            depend = depend.difference(&document.semantics.get_statement_semantics_for_node(node.id()).provide).cloned().collect();
        ...
        match node.kind() {
            "statement" => {
                if node.child_count() >= 1 {
                    ...
                    let mut dependencies = vec![(HashSet::new(), document.semantics.get_statement_semantics_for_node(head.id()).vars)];

                    if (head.kind() == "SHOW" || head.kind() == "EXTERNAL") && node.child_count() >= 2{
                        // We have a show / external statement
                        let term = node.child(1).unwrap();
                        let term_semantics = document.semantics.get_statement_semantics_for_node(term.id());
                        ...
                        Self::update_global_vars_for_node(&document.semantics, term.id(), term_semantics.vars.clone());
                        ...
                        if node.child_count() >= 4 {
                            ...
                            if node.child(2).unwrap().kind() == "COLON" && body.kind() == "bodydot" {
                                dependencies.extend(document.semantics.get_statement_semantics_for_node(body.id()).dependencies)
                            }
                        }
                    }
                        ...
                        if head.kind() == "WIF" {
                            ...
                            let weight_semantics = document.semantics.get_statement_semantics_for_node(weight.id());
    
                            dependencies.push((HashSet::new(), weight_semantics.vars.clone()));
                            Self::update_global_vars_for_node(&document.semantics, weight.id(), weight_semantics.vars.clone());
                            
                            if node.child_count() >= 5 {
                                let tuple = node.child(4).unwrap();
                                let tuple_semantics = document.semantics.get_statement_semantics_for_node(tuple.id());
                                dependencies.push((HashSet::new(), tuple_semantics.vars.clone()));
                                Self::update_global_vars_for_node(&document.semantics, tuple.id(), tuple_semantics.vars.clone());
                            }
                        }
                    }
                    else if node.child_count() >= 3 {
            ...
            "atom" => {
                if node.child_count() >=3 {
                    ...
                    let argument_semantics = document.semantics.get_statement_semantics_for_node(argument.id());
                    let dependencies = vec![(argument_semantics.provide, HashSet::new()), (HashSet::new(), argument_semantics.depend)];
                    ...
                    if node.child_count() >= 3 && atom_id <= node.child_count() - 3 {
                        ...
                        match operator.kind() {
                            "cmp" => {
                                ...
                                let mut dep = Vec::new();
                                let left_semantics = document.semantics.get_statement_semantics_for_node(atom.id());
                                let right_semantics = document.semantics.get_statement_semantics_for_node(right_atom.id());


                                let vars: HashSet<String> = left_semantics.vars.union(&right_semantics.vars).cloned().collect();
                                ...
                                if comparison != "EQ" {
                                    ...
                                    // (∅, dt(t1) ∪ dt(t2))
                                    dep.push((HashSet::new(), left_semantics.depend.union(&right_semantics.depend).cloned().collect()));
                                }
                        ...
                        let mut dep = vec![(HashSet::new(), document.semantics.get_statement_semantics_for_node(atom.id()).vars)];
                        Self::update_dependencies_for_node(&document.semantics, node.id(), dep);
                    }
                }
            }
            "conjunction" => {
                ...
                if node.child_count() >= 3 {
                    ...
                    let local_context = document.semantics.get_statement_semantics_for_node(local_literal.id());
                    let condition_context = document.semantics.get_statement_semantics_for_node(condition.id());
                    Self::update_global_vars_for_node(&document.semantics, node.id(), local_context.vars.difference(&condition_context.vars).cloned().collect());
                }
            }
            //TODO: Check if bodycomma and bodydot cannot be combined also with litvec maybe ?
            "bodycomma" => {
            ...
            "lubodyaggregate" => {
                if node.child_count() >= 2 {
                    ...
                    if let Some(aggr) = aggregate {
                        aggr_vars = document.semantics.get_statement_semantics_for_node(aggr.id()).vars;
                    }
    
                    //Depending if certain bounds exist and their types we now set the dependencies
                    //TODO: see if this can be somehow combined ?
                    if lower_bounds.is_some() && lower_bounds.unwrap() == "EQ" && lower_bounds_term.is_some() {
                        ...
                        let lower_bounds_term_semantics = document.semantics.get_statement_semantics_for_node(lower_bounds_term.unwrap().id());
            ...
            "altheadaggrelemvec" => {
                ...
                if node.child_count() == 1 {
                    ...
                    let terms_semantics = document.semantics.get_statement_semantics_for_node(terms.id());
                    let condition = node.child(1).unwrap();
                    let condition_semantics = document.semantics.get_statement_semantics_for_node(condition.id());
                    ...
                    let left_child = node.child(0).unwrap();
                    let left_semantics = document.semantics.get_statement_semantics_for_node(left_child.id());
                    let operator = node.child(1).unwrap();
                    let right_child = node.child(2).unwrap();
                    let right_semantics = document.semantics.get_statement_semantics_for_node(right_child.id());
                    ...
                    global_vars = global_vars.union(&left_semantics.global_vars).cloned().collect::<HashSet<String>>();
                    ...
                    if node.child_count() >= 4 {
                        ...
                        // if there is an condition only pass on the variables which are not in the condition
                        let vars : HashSet<String> = right_semantics.vars.difference(&condition_semantics.vars).cloned().collect();
                        global_vars.extend(vars);
                        ...
                        //update the special literal semantics
                        special_literals.push(SpecialLiteralSemantics::new_with_dep(&node, local_dependency));
                    } else {
                    ...
                    Self::update_global_vars_for_node(&document.semantics, node.id(), global_vars);
                    Self::update_special_literals_for_node(&document.semantics, node.id(), special_literals);
                } 
            }
            "disjunction" => {
            ...
            "minelemlist" | "maxelemlist" => {
                ...
                if node.child_count() >= 2 {
                    ...
                    if node.child_count() >= 3 {
                        ...
                        Self::update_global_vars_for_node(&document.semantics, tuple.id(), tuple_semantics.vars);
                    } else {
    ...
    fn check_global_vars(node: Node, document: &mut DocumentData) {
        ...
        match node.kind() {
            "literal" => {
                ...
                Self::update_global_vars_for_node(&document.semantics, node.id(), document.semantics.get_statement_semantics_for_node(node.id()).vars);
            }
